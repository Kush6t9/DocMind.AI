...
const SUGGESTIONS = [
  ...
  "List any immediate action items.",
...
export default function ChatInterface({
  ...
  const handleScroll = () => {
    ...
    // Show button if user scrolls up past 200px from bottom
  ...
  const handleSend = async (messageText: string) => {
    ...
    try {
      ...
      const response = await fetch("/api/chat", {
        method: "POST",
        headers,
        body: JSON.stringify({ fileId, message: messageText }),
      });
      ...
      if (!reader) throw new Error("Body reader allocation failed");
  ...
  return (
    <div className="flex flex-col h-full min-h-[450px] md:min-h-[550px] bg-bg-surface border border-slate-800/80 rounded-3xl overflow-hidden shadow-xl relative" id="chat-interface-wrapper">
      {/* Chat Header */}
      ...
      <div
        ...
        className="flex-1 overflow-y-auto p-5 flex flex-col gap-4 relative bg-bg-surface/10"
      >
        {chatHistory.length === 0 ? (
      ...
      <form
        ...
        className="p-4 bg-slate-805/30 border-t border-slate-800/80 flex gap-2 items-center"
      >
        <input
